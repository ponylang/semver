"""
Constraint solver that resolves a set of version constraints against
an artifact source.
"""
...
class Solver
  """
  Resolves a set of version constraints against an artifact source,
  backtracking when conflicts arise.
  """
  ...
  fun ref solve(constraints: Iterator[Constraint]): Result =>
    """
    Finds a set of artifacts satisfying all constraints, or returns
    an error describing the first conflict.
    """
    ...
    while pending_cells.size() > 0 do
      ...
      for pcell in pending_cells.values() do
        ...
        // record its activation with the global list and
        // add its dependencies to the tail of the
        // pending cells.
        ...
        let existing_cell =
          try
            ...
            pcell.picks =
              _all_versions_of(name)
            (let match_index, let ok) =
              _index_of_first_match(
                pcell.picks, [constraint.range])
            if (not ok) then
              return Result(
                where err' =
                  "no artifacts match "
            ...
            _pick(
              pcell, match_index, new_pending_cells)
            pcell
          end

        // New constraint is compatible with existing
        // pick.
        ...
        // New constraint is incompatible with existing
        // pick: log if this is the first such conflict
        // (in case we can't find a solution), then
        // backtrack up the tree until an alternative
        // path is found.
        ...
        while true do
          ...
          | let c: _Cell =>
            let ranges =
              Array[Range]
                .> push(c.constraint.range)

            // Blend in the constraint that kicked
            // this backtracking off.
            if
              conflicting_constraint.artifact_name
  ...
  =>
    ...
    // Reverse sort to make all the 'default to latest'
    // optimizations work.
  ...
  =>
    ...
    cell.children.concat(cells_from_deps.values())
    new_cells.concat(cells_from_deps.values())
