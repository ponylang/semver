...
primitive ParseVersion
  """
  Parses a semver string into a Version, collecting any errors.
  """
  fun apply(s: String): Version =>
    """
    Parses `s` as a semver string and returns the resulting Version.
    """
    let v = Version(0)
    ...
    try
      ...
      let head_and_pre_rel =
        head_and_build(0)?.split("-", 2)

      let maj_min_pat =
        recover box head_and_pre_rel(0)?.split(".") end
      if (maj_min_pat.size() != 3) then
        v.errors.push(
          "expected head of version string to be"
      ...
      if (head_and_pre_rel.size() == 2) then
        for p in
          head_and_pre_rel(1)?.split(".").values()
        do
          if
            (p != "") and
              Strings.contains_only(p, Consts.nums())
          then
            if
              (p.size() > 1) and
                (p.compare_sub("0", 1) is Equal)
            then
              v.errors.push(
      ...
      if (head_and_build.size() == 2) then
        v.build_fields.append(
          head_and_build(1)?.split("."))
      end
    else
    ...
    v.errors.append(
      ValidateFields(v.pr_fields, v.build_fields))
    v
